...
public class DateUtil {
	...
	// 2012.09.07 - S ojw
	public static String addMonths(String str, int months, String retPattern) {
		if (months == 0) {
			return str;
		} 
		DateTimeFormatter fmt = DateTimeFormat.forPattern(DATE_PATTERN_DASH);
		DateTime dt = fmt.parseDateTime(str);
		DateTime subtracted = dt.withFieldAdded(DurationFieldType.months(), months);
		fmt = DateTimeFormat.forPattern(retPattern);
		return fmt.print(subtracted);
	}
	// 2012.09.07 - E ojw
...
//	}
     ...
     * 2012.08.08
     * 
     * 현재 날짜를 기준으로 과거 및 미래 한달 단위 년월 리스트 
     *   
     * @param type : - 이면 현재 기준 과거, + 이면 현재 기준 미래
     * @param format 
     * @param format2
     * @param past : 리스트로 뽑을 월의 수
     * @return
     */
    public static String[][] getYMList(int type, String format ,String format2 , int past){
    	String [][] result = new String[past][2] ;  
    	SimpleDateFormat sdf = new SimpleDateFormat(format);
    	SimpleDateFormat sdf2 = new SimpleDateFormat(format2);
    	Calendar rightNow = Calendar.getInstance();
    	for(int i=0 ; i<past ; i++){
    		result[i][0] = sdf.format(rightNow.getTime());
    		result[i][1] = sdf2.format(rightNow.getTime());
    		rightNow.add(Calendar.MONTH, type) ;  
    	}
    	return result ; 
    }
     ...
     * @return String representing current date (type of pattern)
     */
    public static String getDateString(String pattern, int year, int month, int day) {
    	SimpleDateFormat df = new SimpleDateFormat(pattern);
    	Calendar rightNow = Calendar.getInstance();
    	if(year!=0) rightNow.add(Calendar.YEAR, year) ;  
    	if(month!=0) rightNow.add(Calendar.MONTH, month) ;  
    	if(day!=0) rightNow.add(Calendar.DAY_OF_MONTH, day) ;  
    	return df.format(rightNow.getTime());
    }
    

	public static int DayDifference(String day1, String day2) {

		if(day1==null || day2==null)
			return 0;
		
		Date c1 = string2Date(day1) ;
		Date c2 = string2Date(day2) ; 
		 
		return (int) ((c1.getTime()-c2.getTime())/ 1000) / (3600*24) ;  
	}
	

	public static int DayDifference(String day1, String day2, String foramt) {

		if(day1==null || day2==null)
			return 0;
		
		Date c1 = string2Date(day1, foramt) ;
		Date c2 = string2Date(day2, foramt) ;
